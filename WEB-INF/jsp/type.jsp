...
<s:form action="type" id="form_id" method="post" >
	...
	<s:if test="id == ''">
		<h1>New <s:property value="typesTitle" /></h1>
	</s:if>
	<s:else>
  ...
  <p>* Required field <br />
		...
		<s:else>
			Pick a field set from list below to add or update an item in the set.<br />
			You must hit 'Save' button to save data.<br />
			If certain item is not in use anymore, you can turn it to 'Inactive'.
		</s:else>
	</p>
	<div class="tt-row-container">
		<dl class="fn1-output-field">
			<dt>Field Set </dt>
			<dd><s:select id="selection_id" name="selection" value="%{selection}" list="#{'types':'Project Types','funding_sources':'Funding Sources','ranks':'Phase Ranks','contractors':'Contractors/Sub-contractors','consultants':'Consultants/Sub-consultants','locations':'Locations'}" /> </dd>
		</dl>		
		<s:if test="type.id != ''">
			<dl class="fn1-output-field">
				<dt>ID </dt>
